...
const PageHead = () => (
  <div style={{ padding: '18px 20px 0' }}>
    <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase' as const, color: 'rgba(40,57,108,.5)', marginBottom: 6 }}>Parent Dashboard</div>
    <div style={{ fontSize: 26, fontWeight: 700, color: '#1e2d57', letterSpacing: '-.6px', lineHeight: 1.1 }}>My Classes</div>
    <div style={{ fontSize: 13, color: 'rgba(40,57,108,.45)', marginTop: 4 }}>All enrolled subjects for <strong style={{ color: '#28396c', fontWeight: 600 }}>Tanveer Sultana</strong></div>
  </div>
);
...
const ClassCard = ({ theme, subject, subjectIcon, teacherInit, teacherName, teacherBg }: ClassCardProps) => (
  <div style={{ margin: '18px 16px 0', borderRadius: 24, overflow: 'hidden', boxShadow: '0 0 0 .5px rgba(40,57,108,.08), 0 4px 18px rgba(40,57,108,.09), 0 22px 52px rgba(40,57,108,.13)' }}>
    ...
    <div style={{ background: theme, padding: '18px 18px 20px', position: 'relative', overflow: 'hidden', minHeight: 130 }}>
      ...
      <div style={{ position: 'absolute', top: 14, right: 14, display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 100, background: 'rgba(255,255,255,.18)', border: '.5px solid rgba(255,255,255,.3)', fontSize: 10, fontWeight: 700, color: '#fff' }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 700, color: '#fff', letterSpacing: '-.3px', marginBottom: 8 }}>Class</div>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 100, background: 'rgba(255,255,255,.15)', border: '.5px solid rgba(255,255,255,.22)', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,.9)', backdropFilter: 'blur(8px)' }}>
        {subjectIcon}{subject}
      </div>
    </div>
    {/* Body */}
    <div style={{ background: '#fff', padding: '16px 16px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 14, borderBottom: '.5px solid rgba(40,57,108,.07)', marginBottom: 14 }}>
        <div style={{ width: 42, height: 42, borderRadius: 13, background: teacherBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', flexShrink: 0, boxShadow: '0 4px 14px rgba(0,0,0,.15)' }}>{teacherInit}</div>
        <div><div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.09em', textTransform: 'uppercase' as const, color: 'rgba(40,57,108,.35)' }}>Teacher</div><div style={{ fontSize: 15, fontWeight: 700, color: '#1e2d57', letterSpacing: '-.2px', marginTop: 2 }}>{teacherName}</div></div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
        <div style={{ background: '#fdfaf4', borderRadius: 12, padding: '11px 12px', border: '.5px solid rgba(40,57,108,.07)' }}>
          <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(40,57,108,.35)', marginBottom: 4 }}>Schedule</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#1e2d57' }}>08:30 – 09:30 AM</div>
        </div>
        <div style={{ background: '#fdfaf4', borderRadius: 12, padding: '11px 12px', border: '.5px solid rgba(40,57,108,.07)' }}>
          <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(40,57,108,.35)', marginBottom: 4 }}>Year</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#1e2d57' }}>2025–26</div>
        </div>
      </div>
      <div style={{ margin: '0 -16px', padding: 14, background: theme, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 700, color: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="sp-scroll">
        ...
        <div style={{ margin: '14px 16px 0', background: 'linear-gradient(135deg,#28396c,#1e2d57)', borderRadius: 20, padding: 18, position: 'relative', overflow: 'hidden', boxShadow: '0 8px 32px rgba(40,57,108,.25)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, position: 'relative', zIndex: 1 }}>
            ...
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.45)', marginBottom: 3 }}>Enrollment Verified</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#fff', letterSpacing: '-.2px' }}>Academic Registry Active</div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'rgba(255,255,255,.08)', borderRadius: 14, overflow: 'hidden', position: 'relative', zIndex: 1 }}>
            <div style={{ background: 'rgba(255,255,255,.04)', padding: '14px 16px' }}>
              <div style={{ fontSize: 28, fontWeight: 700, color: '#fff', letterSpacing: '-1px', lineHeight: 1 }}>2</div>
              <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: '.09em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.4)', marginTop: 3 }}>Subjects</div>
            </div>
            <div style={{ background: 'rgba(255,255,255,.04)', padding: '14px 16px' }}>
              <div style={{ fontSize: 28, fontWeight: 700, color: '#fff', letterSpacing: '-1px', lineHeight: 1 }}>2</div>
              <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: '.09em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.4)', marginTop: 3 }}>Teachers</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '18px 20px 0' }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(40,57,108,.38)' }}>Enrolled Teachers</div>
        ...
        <div style={{ margin: '10px 16px 0', background: '#fff', borderRadius: 18, overflow: 'hidden', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.06)' }}>
          {[
            ...
            <div key={t.ini} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', borderBottom: i === 0 ? '.5px solid rgba(40,57,108,.07)' : 'none' }}>
              <div style={{ width: 42, height: 42, borderRadius: 13, background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', flexShrink: 0 }}>{t.ini}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#1e2d57' }}>{t.name}</div>
