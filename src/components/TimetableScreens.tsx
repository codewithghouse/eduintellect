...
const HeroBanner = ({ todayCount }: { todayCount: number }) => (
  <div style={{ margin: '14px 16px 0', borderRadius: 24, background: 'linear-gradient(140deg,#28396c,#3d5494)', padding: '22px 20px 24px', position: 'relative', overflow: 'hidden', boxShadow: '0 0 0 .5px rgba(40,57,108,.08), 0 4px 18px rgba(40,57,108,.09), 0 22px 52px rgba(40,57,108,.13)' }}>
    ...
    <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.5)', marginBottom: 8, position: 'relative', zIndex: 1 }}>Weekly Schedule</div>
    <div style={{ fontSize: 32, fontWeight: 700, color: '#fff', letterSpacing: '-.7px', lineHeight: 1.1, position: 'relative', zIndex: 1, marginBottom: 8 }}>Timetable</div>
    ...
    <div style={{ display: 'flex', gap: 0, marginTop: 18, background: 'rgba(255,255,255,.08)', borderRadius: 14, overflow: 'hidden', border: '.5px solid rgba(255,255,255,.1)', position: 'relative', zIndex: 1 }}>
      {[{ v: '5', l: 'School Days' }, { v: '2', l: 'Subjects' }, { v: String(todayCount), l: 'Today' }].map((s, i) => (
        <div key={s.l} style={{ flex: 1, padding: '11px 12px', borderRight: i < 2 ? '.5px solid rgba(255,255,255,.08)' : 'none' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#fff', letterSpacing: '-.4px', lineHeight: 1 }}>{s.v}</div>
          <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.38)', marginTop: 3 }}>{s.l}</div>
        </div>
      ))}
    </div>
  </div>
      ...
      {days.map((d, i) => {
        ...
        return (
          <div key={d.name} style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 3, padding: '10px 12px', borderRadius: 16, background: isActive ? 'linear-gradient(140deg,#28396c,#334880)' : '#fff', border: isActive ? 'none' : '.5px solid rgba(40,57,108,.07)', flex: 1, boxShadow: isActive ? '0 4px 16px rgba(40,57,108,.28)' : '0 1px 4px rgba(40,57,108,.05)', transform: isActive ? 'scale(1.04)' : 'none' }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase' as const, color: isActive ? 'rgba(255,255,255,.7)' : '#8892b0' }}>{d.name}</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: isActive ? '#fff' : '#1a2340', letterSpacing: '-.2px', lineHeight: 1 }}>{d.num}</div>
            {isActive && <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.6)', marginTop: 1 }}>Today</div>}
    ...
    <div className="sp">
      ...
      <div className="sp-scroll">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 0' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#1a2340', letterSpacing: '-.4px' }}>Wednesday</div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 100, background: 'rgba(40,57,108,.09)', border: '.5px solid rgba(40,57,108,.14)', fontSize: 10, fontWeight: 700, color: '#28396c' }}>0 Periods</div>
        </div>
        {/* Empty state */}
        <div style={{ margin: '16px 16px 0', background: '#fff', borderRadius: 24, padding: '44px 28px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.07)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: '#1a2340', letterSpacing: '-.3px', marginBottom: 6, textAlign: 'center' as const }}>No Classes Today</div>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 100, background: 'rgba(46,188,113,.11)', border: '.5px solid rgba(46,188,113,.2)', fontSize: 12, fontWeight: 700, color: '#2ebc71', marginTop: 14 }}>
  ...
  <div className="sp-wrap">
    ...
    <div className="sp">
      ...
      <div className="sp-scroll">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 0' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#1a2340', letterSpacing: '-.4px' }}>Monday</div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 100, background: 'rgba(40,57,108,.09)', border: '.5px solid rgba(40,57,108,.14)', fontSize: 10, fontWeight: 700, color: '#28396c' }}>2 Periods</div>
        </div>
            ...
            <div key={p.subject} style={{ display: 'flex', alignItems: 'stretch', borderBottom: i === 0 ? '.5px solid rgba(40,57,108,.07)' : 'none' }}>
              ...
              <div style={{ flex: 1, padding: '13px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ minWidth: 44, flexShrink: 0, textAlign: 'center' as const }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#1a2340', letterSpacing: '-.1px' }}>8:30</div>
                ...
                <div style={{ flex: 1, paddingLeft: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#1a2340', letterSpacing: '-.1px' }}>{p.subject}</div>
                  <div style={{ fontSize: 11, color: '#8892b0', marginTop: 2 }}>{p.teacher}</div>
                </div>
                <div style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 9px', borderRadius: 100, fontSize: 10, fontWeight: 600, background: p.roomBg, color: p.roomColor, border: `.5px solid ${p.roomBdr}`, flexShrink: 0 }}>{p.room}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ margin: '12px 16px 0', background: '#fff', borderRadius: 18, padding: '14px 16px', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.07)' }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase' as const, color: '#c0c8dc', marginBottom: 12 }}>This Week</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 5 }}>
            {[
              ...
              <div key={w.d} style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 5 }}>
                <div style={{ width: 30, height: 30, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, letterSpacing: '.04em', textTransform: 'uppercase' as const, ...(w.type === 'today' ? { background: '#28396c', color: '#fff', boxShadow: '0 3px 10px rgba(40,57,108,.25)' } : { background: '#f5efe2', color: '#c0c8dc', border: '.5px solid rgba(40,57,108,.07)' }) }}>{w.d}</div>
                <div style={{ fontSize: 8, color: '#c0c8dc', fontWeight: 600 }}>{w.lbl}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ margin: '12px 16px 0', background: 'linear-gradient(135deg,#28396c,#1e2d57)', borderRadius: 18, padding: '16px 16px', position: 'relative', overflow: 'hidden', boxShadow: '0 6px 24px rgba(40,57,108,.22)' }}>
          ...
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.45)', marginBottom: 12, position: 'relative', zIndex: 1 }}>Upcoming Classes</div>
          {[
            ...
            <div key={u.day} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: i === 0 ? '.5px solid rgba(255,255,255,.07)' : 'none', position: 'relative', zIndex: 1 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: 'rgba(255,255,255,.1)', border: '.5px solid rgba(255,255,255,.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,.7)', flexShrink: 0, textTransform: 'uppercase' as const }}>{u.day}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#fff', letterSpacing: '-.1px' }}>{u.subject}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,.45)', marginTop: 2 }}>{u.time}</div>
              </div>
              <div style={{ padding: '3px 8px', borderRadius: 100, fontSize: 10, fontWeight: 700, background: 'rgba(255,255,255,.1)', color: 'rgba(255,255,255,.6)', border: '.5px solid rgba(255,255,255,.1)' }}>1</div>
            </div>
          ))}
        </div>
        <div style={{ height: 20 }} />
