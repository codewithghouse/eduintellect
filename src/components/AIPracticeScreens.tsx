...
const SPHeader = () => (
  <div className="sp-hdr">
    <div className="sp-brand">
      ...
      <span className="sp-brand-txt">Edullent</span>
    </div>
        ...
        <div className="sp-scroll">
          ...
          <div style={{ margin: '16px 16px 0', borderRadius: 24, background: 'linear-gradient(140deg,#28396c 0%,#334880 60%,#4a5db8 100%)', padding: '20px 20px 22px', position: 'relative', overflow: 'hidden', boxShadow: '0 0 0 .5px rgba(40,57,108,.08), 0 4px 18px rgba(40,57,108,.09), 0 24px 56px rgba(40,57,108,.13)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 100, background: 'rgba(255,255,255,.14)', border: '.5px solid rgba(255,255,255,.22)', fontSize: 9, fontWeight: 300, color: '#fff', letterSpacing: '.04em', marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 26, fontWeight: 300, color: '#fff', letterSpacing: '-.7px', lineHeight: 1.12, marginBottom: 6 }}>AI Practice<br />Exams</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
                {[
                  ...
                  <div key={s.lbl} style={{ background: 'rgba(255,255,255,.1)', border: '.5px solid rgba(255,255,255,.16)', borderRadius: 16, padding: '12px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 300, color: '#fff', letterSpacing: '-.4px', lineHeight: 1 }}>{s.val}</div>
                    <div style={{ fontSize: 8, fontWeight: 300, letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,.45)' }}>{s.lbl}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ margin: '12px 16px 0', display: 'flex', alignItems: 'center', gap: 12, background: 'linear-gradient(135deg,rgba(245,197,66,.08),rgba(245,197,66,.04))', borderRadius: 16, padding: '12px 16px', border: '.5px solid rgba(245,197,66,.2)' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 300, color: '#1a2340', letterSpacing: '-.2px' }}>Practice Streak</div>
              <div style={{ fontSize: 11, color: '#8892b0', marginTop: 2 }}>Start today to build your streak!</div>
            </div>
            <div style={{ fontSize: 22, fontWeight: 300, color: '#f5c542', letterSpacing: '-.5px' }}>0d</div>
          </div>
          ...
          <div style={{ margin: '14px 16px 0', background: '#fff', borderRadius: 20, padding: '16px 18px', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.06)' }}>
            <div style={{ fontSize: 15, fontWeight: 300, color: '#1a2340', letterSpacing: '-.3px' }}>Practice Calendar</div>
          ...
          <div style={{ margin: '14px 16px 0', background: 'linear-gradient(135deg,#28396c,#1e2d57)', borderRadius: 16, padding: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 300, color: '#fff', letterSpacing: '-.1px', position: 'relative', overflow: 'hidden', boxShadow: '0 6px 24px rgba(40,57,108,.28), 0 2px 8px rgba(0,0,0,.1)' }}>
          ...
          <div style={{ margin: '10px 16px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={e.name} style={{ background: '#fff', borderRadius: 16, padding: '12px 14px', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06)', border: '.5px solid rgba(40,57,108,.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 300, color: '#1a2340', letterSpacing: '-.2px' }}>{e.name}</div>
                  <div style={{ fontSize: 10, color: '#8892b0', marginTop: 2 }}>{e.meta}</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                  <div style={{ fontSize: 16, fontWeight: 300, color: e.pctColor, letterSpacing: '-.4px' }}>{e.pct}</div>
                  <div style={{ padding: '2px 8px', borderRadius: 100, fontSize: 9, fontWeight: 300, background: e.bg, color: e.pctColor, border: `.5px solid ${e.bdr}` }}>{e.chip}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="sp">
        ...
        <div className="sp-scroll">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '12px 20px 0', cursor: 'pointer', width: 'fit-content' }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 400, color: '#28396c', letterSpacing: '-.1px' }}>Back</span>
          </div>

          {/* Upload Head */}
          <div style={{ padding: '16px 20px 0' }}>
            <div style={{ fontSize: 22, fontWeight: 300, color: '#1a2340', letterSpacing: '-.5px' }}>Upload Syllabus</div>
          ...
          <div style={{ margin: '18px 16px 0', border: '2px dashed rgba(40,57,108,.22)', borderRadius: 22, padding: '42px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, background: 'linear-gradient(135deg,rgba(40,57,108,.025),rgba(40,57,108,.01))', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 300, color: '#28396c', letterSpacing: '-.2px', position: 'relative', zIndex: 1 }}>Drop PDF here</div>
            ...
            <div style={{ fontSize: 10, fontWeight: 400, color: '#c0c8dc', marginTop: 1, position: 'relative', zIndex: 1 }}>Max 20 MB · PDF, DOC, DOCX</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, margin: '14px 16px 0' }}>
            {[
              ...
              <div key={o.lbl} style={{ background: '#fff', borderRadius: 16, padding: '14px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06)', border: '.5px solid rgba(40,57,108,.06)' }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 300, color: '#1a2340', letterSpacing: '-.2px', textAlign: 'center' }}>{o.lbl}</div>
          ...
          <div style={{ display: 'flex', gap: 6, margin: '14px 16px 0', flexWrap: 'wrap' }}>
            {[
              ...
              <div key={p.lbl} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 100, fontSize: 10, fontWeight: 300, background: p.bg, color: p.color, border: `.5px solid ${p.bdr}` }}>
          ...
          {/* Upload Button */}
          <div style={{ margin: '14px 16px 0', background: 'linear-gradient(135deg,#28396c,#1e2d57)', borderRadius: 16, padding: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 300, color: '#fff', position: 'relative', overflow: 'hidden', boxShadow: '0 6px 24px rgba(40,57,108,.28)' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,rgba(255,255,255,.1) 0%,transparent 55%)' }} />
