...
const PageHead = () => (
  <div style={{ padding: '18px 20px 0' }}>
    <div style={{ fontSize: 24, fontWeight: 300, color: '#1a2340', letterSpacing: '-.5px', lineHeight: 1.15 }}>Assignments &<br />Coursework</div>
      ...
      <div className="sp-scroll">
        ...
        <div style={{ overflow: 'hidden', paddingTop: 14 }}>
          <div style={{ display: 'flex', gap: 8, padding: '0 20px', overflowX: 'auto' as const }}>
            {[
              ...
              <div key={s.lbl} style={{ background: '#fff', borderRadius: 18, padding: '14px 16px', display: 'flex', flexDirection: 'column' as const, gap: 3, boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.06)', flexShrink: 0, minWidth: 110 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ fontSize: 8, fontWeight: 300, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: '#c0c8dc' }}>{s.lbl}</div>
                ...
                <div style={{ fontSize: 22, fontWeight: 300, letterSpacing: '-.6px', lineHeight: 1, color: s.color }}>{s.val}</div>
            ...
            <div style={{ background: '#28396c', borderRadius: 18, padding: '14px 16px', display: 'flex', flexDirection: 'column' as const, gap: 3, boxShadow: '0 6px 22px rgba(40,57,108,.28)', flexShrink: 0, minWidth: 110 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontSize: 8, fontWeight: 300, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.45)' }}>Pending</div>
              ...
              <div style={{ fontSize: 22, fontWeight: 300, letterSpacing: '-.6px', lineHeight: 1, color: '#fff' }}>0</div>
        ...
        <div style={{ margin: '16px 16px 0', background: '#f5efe2', borderRadius: 16, padding: 3, display: 'flex', gap: 2, border: '.5px solid rgba(40,57,108,.07)' }}>
          {[{ t: 'Pending', c: '0', a: true }, { t: 'Done', c: '0' }, { t: 'Overdue', c: '0' }].map((b) => (
            <div key={b.t} style={{ flex: 1, padding: '9px 4px', borderRadius: 13, fontSize: 11, fontWeight: 300, textAlign: 'center' as const, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, ...(b.a ? { background: 'linear-gradient(135deg,#28396c,#334880)', color: '#fff', boxShadow: '0 3px 12px rgba(40,57,108,.25)' } : { color: '#8892b0' }) }}>
              {b.t}
              <div style={{ minWidth: 16, height: 16, borderRadius: 8, fontSize: 9, fontWeight: 300, padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: b.a ? 'rgba(255,255,255,.2)' : 'rgba(40,57,108,.1)', color: b.a ? '#fff' : '#4a5578' }}>{b.c}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ margin: '12px 16px 0', background: '#fff', borderRadius: 24, padding: '44px 28px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.06)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 300, color: '#1a2340', letterSpacing: '-.3px', marginBottom: 6, textAlign: 'center' as const }}>All Caught Up!</div>
          ...
          <div style={{ marginTop: 20, padding: '12px 24px', background: 'linear-gradient(135deg,#28396c,#334880)', borderRadius: 14, fontSize: 13, fontWeight: 300, color: '#fff', boxShadow: '0 6px 22px rgba(40,57,108,.28)', display: 'flex', alignItems: 'center', gap: 8, position: 'relative', overflow: 'hidden' }}>
    ...
    <div className="sp">
      ...
      <div className="sp-scroll">
        ...
        <div style={{ margin: '14px 16px 0', background: 'linear-gradient(140deg,#28396c,#1e2d57)', borderRadius: 20, padding: 16, position: 'relative', overflow: 'hidden', boxShadow: '0 0 0 .5px rgba(40,57,108,.08), 0 4px 18px rgba(40,57,108,.09), 0 24px 56px rgba(40,57,108,.12)' }}>
          ...
          <div style={{ fontSize: 9, fontWeight: 300, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.45)', marginBottom: 12, position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 6 }}>This Term Overview<div style={{ flex: 1, height: '.5px', background: 'rgba(255,255,255,.1)' }} /></div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 1, background: 'rgba(255,255,255,.07)', borderRadius: 14, overflow: 'hidden', position: 'relative', zIndex: 1 }}>
            {[{ v: '93%', l: 'Completion' }, { v: '96%', l: 'On-Time' }, { v: '82%', l: 'Avg Score' }].map((c) => (
              <div key={c.l} style={{ background: 'rgba(255,255,255,.04)', padding: '12px 8px', display: 'flex', flexDirection: 'column' as const, gap: 3, alignItems: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 300, color: '#fff', letterSpacing: '-.5px', lineHeight: 1 }}>{c.v}</div>
                <div style={{ fontSize: 8, fontWeight: 300, letterSpacing: '.08em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,.4)' }}>{c.l}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {assignCards.map((a, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 18, padding: '14px 16px', boxShadow: '0 0 0 .5px rgba(40,57,108,.06), 0 2px 8px rgba(40,57,108,.06), 0 10px 28px rgba(40,57,108,.08)', border: '.5px solid rgba(40,57,108,.06)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 9, paddingLeft: 6 }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 300, letterSpacing: '.08em', textTransform: 'uppercase' as const, color: '#c0c8dc', marginBottom: 3 }}>{a.subject}</div>
                  <div style={{ fontSize: 14, fontWeight: 300, color: '#1a2340', letterSpacing: '-.2px', lineHeight: 1.3 }}>{a.title}</div>
                </div>
                <div style={{ padding: '4px 10px', borderRadius: 100, fontSize: 10, fontWeight: 300, background: a.chipBg, color: a.chipC, border: `.5px solid ${a.chipBdr}`, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4 }}>{a.chipTxt}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 6, paddingTop: 8, borderTop: '.5px solid rgba(40,57,108,.07)', flexWrap: 'wrap' as const }}>
                ...
                <div style={{ fontSize: 10, fontWeight: 400, color: a.rightC, marginLeft: 'auto' }}>{a.right}</div>
              </div>
            </div>
          ))}
        </div>
